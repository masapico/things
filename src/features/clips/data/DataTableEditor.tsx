...
export function DataTableEditor({ document, selection, viewSelection, onSelectionChange, onCellChange, onColumnChange, onPaste }: Props) {
  ...
  return <div className="data-grid-wrap" tabIndex={0} onPaste={(event) => {
    if (event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement || event.target instanceof HTMLSelectElement) return;
    const text = event.clipboardData.getData("text/plain");
    if (!text) return;
    event.preventDefault(); onPaste(text);
  }}><table className="data-grid"><thead><tr><th className="data-grid-corner">#</th>{document.columns.map((column, index) => <th key={column.id} className={selection.mode === "columns" && index >= selection.columnStart && index <= selection.columnEnd ? "is-axis-selected" : ""}>
